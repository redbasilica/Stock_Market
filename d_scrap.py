...
def scrape_stock_data():
    """
    Scrapes stock data from Ä°ÅŸ YatÄ±rÄ±m website and returns as DataFrame
    """
    ...
    try:
        print("Connecting to Ä°ÅŸ YatÄ±rÄ±m website...")
...
def update_dataframe_types(df_ozet, df_temp):
    ...
    if 'Kod' in df_ozet.columns and 'Borsa Kodu' in df_temp.columns and 'Fiili DolaÅŸÄ±mdaki Pay TutarÄ±(TL)' in df_temp.columns:
        # Create a mapping dictionary from df_temp
        kap_mapping = df_temp.set_index('Borsa Kodu')['Fiili DolaÅŸÄ±mdaki Pay TutarÄ±(TL)'].to_dict()

        # Add the new column to df_ozet
        df_ozet['Fiili DolaÅŸÄ±mdaki Pay TutarÄ±(TL)'] = df_ozet['Kod'].map(kap_mapping)

        # Convert to integer by rounding up (ceiling)
        df_ozet['Fiili DolaÅŸÄ±mdaki Pay TutarÄ±(TL)'] = df_ozet['Fiili DolaÅŸÄ±mdaki Pay TutarÄ±(TL)'].apply(
            lambda x: int(np.ceil(clean_numeric_value(x))) if not pd.isna(clean_numeric_value(x)) else np.nan
        )

        print("Successfully added 'Fiili DolaÅŸÄ±mdaki Pay TutarÄ±(TL)' column from KAP data.")
    else:
    ...
    integer_columns = ["KapanÄ±ÅŸ(TL)", "Piyasa DeÄŸeri(mn TL)", "Piyasa DeÄŸeri(mn $)", "Sermaye(mn TL)"]
    ...
    # Convert Halka AÃ§Ä±klÄ±k OranÄ± (%) to percentage
    percentage_column = "Halka AÃ§Ä±klÄ±kOranÄ± (%)"
...
def main():
    ...
    """
    # Scrape the stock data from Ä°ÅŸ YatÄ±rÄ±m
    ...
    if export_success:
        print("\nâœ“ Data successfully exported to Excel with date in filename")
    else:
        print("\nâœ— Failed to export data to Excel")
