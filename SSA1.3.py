...
st.title("📈 Hisse Analiz - Hızlandırılmış Versiyon")
...
with col1:
    ma_tolerance = st.slider("MA Yakınlık (%)", 1, 10, 5) / 100
    volume_threshold = st.slider("Hacim Artış (kat)", 0.0, 5.0, 1.5)
...
selected_tickers = st.sidebar.multiselect(
    "📌 Tarama İçin Hisse Seç",
    options=ticker_options,
    help="Boş bırakırsanız tüm hisseler taranır"
)

# Performance settings
st.sidebar.markdown("---")
st.sidebar.markdown("⚡ **Performans Ayarları**")
max_workers = st.sidebar.slider("Paralel İşlem Sayısı", 5, 20, 10)
...
# Footer
st.markdown("---")
st.markdown("*💡 İpucu: Daha hızlı tarama için paralel işlem sayısını artırabilirsiniz.*")